...
btn_logout.onclick = async () => {
  ...
  function createGenreCard(genre) {
    return `<div class="col-sm-12 col-md-12" style="max-width: 300px;">
      <div class="card w-100 mt-3" data-id="${genre.id}">
        <div class="row">
          <div class="col-sm-8">
            <div class="card-body d-flex 18-em">
  ...
  const deleteAction = async (e) => {
    ...
    if (response.ok) {
      successNotification("Genre deleted successfully.", 3);
      getGenres(); // Reload Genres after deletion
    } else {
      errorNotification("Unable to delete genre!", 3);
    }
  };
  ...
  document.getElementById("get_pagination").addEventListener("click", function (e) {
    ...
    if (e.target.tagName === "A" && !e.target.classList.contains("active")) {
      ...
      getGenres(backendURL + "/api/genre?page=" + currentPage);
    }
  });
